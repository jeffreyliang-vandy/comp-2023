...
-   \href{https://vanderbilt365-my.sharepoint.com/:b:/g/personal/zhuohui_liang_vanderbilt_edu/ESFMUceMOKRDkzTV2dPVhDMBZ4-15iHkKeJklYNO6Sm8XA?e=yLNMxv}{Amber's Homework 6(Pretty Bad)}
...
-   \href{https://vanderbilt365-my.sharepoint.com/:b:/g/personal/zhuohui_liang_vanderbilt_edu/EcpcWjslfqFMhSFDV1acnu4BlsC8W3FYBvJXu-OGRAlYdg?e=wUYtoO}{Amber's Homework 5}

## Transformation/Location Scale Family

-   \href{https://vanderbilt365-my.sharepoint.com/:b:/g/personal/zhuohui_liang_vanderbilt_edu/EeDIvgbReHtEvv3cmyCIVNQBEMzJBaP6oIAeWnsE8JxqSA?e=5sb2dT}{Amber's Homework 8}

-   \href{https://vanderbilt365-my.sharepoint.com/:b:/g/personal/zhuohui_liang_vanderbilt_edu/EWekEOvn5kxEuhP7a7t3sUwBnw3N9CdC4op28I5UCbhe3g?e=WaG2Sk}{Amber's Homework 7}
...
```{=tex}
(a)
$X\sim Geometric(p),Y\sim Geometric(p), Y \perp X,$ Show that $U - min(X,Y), V = X-Y$ is independent:

$$
f_{X,Y} = p(1-p)^{x-1} p(1-p)^{y-1} I_{1,2...}(x)I_{1,2...}(y)
$$
\begin{itemize}
\item when $x<y$
...
\textbf{IT'S DISCRETE, NO JACOBIAN}

$$
\begin{gathered}
P(U=u,V=v) = p(1-p)^{u-1} p(1-p)^{u-v-1}I_{1,2...}(u) I_{-1,-2...}(v)\\
= p^2(1-p)^{2u-1}I_{1,2...}(u) (1-p)^{-v-1}I_{-1,-2...}(v)
\end{gathered}
$$
...
\begin{gathered}
P(U=u,V=v) = p(1-p)^{u-1} p(1-p)^{u-v-1}I_{1,2...}(u) I_{-1,-2...}(v)\\
= p^2(1-p)^{2u-2}I_{1,2...}(u) I_{-1,-2...}(v)
\end{gathered}
$$
\textbf{see that indicator function is necessary to show independent, don't be lazy}
\end{itemize}

So $U \perp V$ by factorization.
```
```{=tex}
(b) Find the distribution of $Z = X/(X + Y)$, where we define $Z=O$if $X+Y =0.$
...
$$
\begin{gathered}
P_{V,Z} = p(1-p)^{v-1} p(1-p)^{v(1-z)/z -1} I_{1,...}(v) I_{(0,1)}(z)\\
\Rightarrow
P_Z = \sum_v^\infty P_{V=v,Z}\\
= \frac{p}{1-p}^2 \sum_v (1-p)^{v/Z}\\
= \frac{p}{1-p}^2 \sum_v [(1-p)^{1/Z}]^v\\
\end{gathered}
$$

Now, let $Y = V-1$, such that $Y \in \{0,1,...\}$

By $\sum_{v=0}^\infty r^k = \frac{1}{1-r}$:

$$
\begin{gathered}
P_Z = \frac{p}{1-p}^2 (1-p)^{1/Z}\sum_v [(1-p)^{1/Z}]^{v-1}\\
= \frac{p}{1-p}^2 (1-p)^{1/Z}\sum_v [(1-p)^{1/Z}]^{y}\\
= \frac{p}{1-p}^2 (1-p)^{1/Z}\frac{1}{1-(1-p)^{1/Z}} I_{(0,1)}(z)
\end{gathered}
$$
```
#### Homework7 4.19
...
\href{https://vanderbilt365-my.sharepoint.com/:b:/g/personal/zhuohui_liang_vanderbilt_edu/EVTsbxaxHDJIknUwiJP7HFQB4m7_8pXbKOfeD1CeSghvWA?e=Zht9D5}{Amber's Homework9}
...
$$
|E[XY]| \le E(|XY|) \le
[E(|X|^p)]^{\frac{1}{q}}[E(|Y|^q)]^{\frac{1}{p}}
$$

$$
\Rightarrow \sum|X_iY_i| \le 
[(\|X|^p)]^{\frac{1}{q}}[E(|Y|^q)]^{\frac{1}{p}}
$$
```
### Cauchy's Inequality
...
$$
|E(XY)| \le E(|XY|) \le E[(X^2)]^{\frac{1}{2}}E[(Y^2)]^{\frac{1}{2}}
$$

Use this to show that $|cov(X,Y)|\le \sigma_x\sigma_y$

$$
\begin{gathered}
\Rightarrow |cov(X,Y)| = |E[(X-\mu_x)(Y-\mu_y)]|\\
\le E[|(X-\mu_x)(Y-\mu_y)|]\\
\le E[(X-\mu_x)^2]^{\frac{1}{2}} * E[(Y-\mu_y)^2]^{\frac{1}{2}}\\
= \sigma_x \sigma_y
\end{gathered}
$$

$$
\Rightarrow |\rho(x,y)| \le 1
$$
```
### Jenson's Inequality
...
To establish the inequality, let $l(x)$ be a tangent line to $g(x)$ at the point $g(\mathrm{E} X)$. (Recall that $\mathrm{E} X$ is a constant.) Write $l(x)=a+b x$ for some $a$ and $b$. The situation is illustrated in Figure 4.7.2.
...
The function $g(x)$ is convex if $g^{\prime \prime}(x) \geq 0$, for all $x$, and $g(x)$ is concave if $g^{\prime \prime}(x) \leq 0$, for all $x$. 

Jensen's Inequality applies to concave functions as well. If $g$ is concave, then $\mathrm{E} g(X) \leq g(\mathrm{E} X)$.
```
...
$$
P\left(\left|\bar{X}_{n}-\mu\right| \geq \epsilon\right)=P\left(\left(\bar{X}_{n}-\mu\right)^{2} \geq \epsilon^{2}\right) \leq \frac{\mathrm{E}\left(\bar{X}_{n}-\mu\right)^{2}}{\epsilon^{2}}=\frac{\operatorname{Var} \bar{X}_{n}}{\epsilon^{2}}=\frac{\sigma^{2}}{n \epsilon^{2}}
$$
...
Theorem 5.5.9 (Strong Law of Large Numbers) Let $X_1, X_2, \ldots$ be iid random variables with $\mathrm{E} X_i=\mu$ and Var $X_i=\sigma^2<\infty$, and define $X_n=(1 / \mathrm{n}) \sum_{i=1}^n X_i$. Then, for enery $c>0$,

$$
P\left(\lim _{n \rightarrow \infty}\left|\hat{X}_n-\mu\right|<\epsilon\right)=1
$$

that is, $X_n$ converges almost surely to $\mu$. For both the Weak and Strong Laws of Large Numbers we had the sssumption of a finite variance. Although such an assumption is true (and desirable) in most applications, it is, in fact, a stronger assumption than is needed. Both the weak and strong laws hold without this assumption. The only moment condition needed is that E $\left|X_i\right|<\infty$ (see Resnick 1999, Chapter 7, or Billingsiey 1995, Section 22).
...
(a) Show that $P\left(S_n>a\right) \leq e^{-a t}\left[M_X(t)\right]^n$, for $0<t<h$, and $P\left(S_n \leq a\right) \leq$ $e^{-a t}\left[M_X(t)\right]^n$, for $-h<t<0$
(b) Use the facts that $M_X(0)=1$ and $M_X^{\prime}(0)=\mathrm{E}(X)$ to show that, if $\mathrm{E}(X)<0$, then there is a $0<c<1$ with $P\left(S_n>a\right) \leq c^n$. Establish a similar bound for $P\left(S_n \leq a\right)$
(c) Define $Y_i=X_i-\mu-\varepsilon$ and use the above argument, with $a=0$, to establish that $P\left(\bar{X}_n-\mu>\varepsilon\right) \leq c^n$.
(d) Now define $Y_i=-X_i+\mu-\varepsilon$, establish an equality similar to part (c), and combine the two to get
$$
P\left(\left|\bar{X}_n-\mu\right|>\varepsilon\right) \leq 2 c^n \text { for some } 0<c<1
$$

HINT: Hint: For part (b) you may assume that a > 0 and establish the upper bound on P(Sn>a). Use the definition Mx(0) = lim Mx() - Mx(0) - do to show that there exists 8 >0 such that Mx(8) < 1. Use this result to define a suitable c and apply part (a). You may stop here since the result for a > 0 is all that is needed to complete parts (c) and (d). The trick to getting the upper bound on P(Sn <a) is to assume that E(X) > 0. For parts (c) and (d), apply (b) with a = 0.
```
## CLT
...
[*Thought process: since to prove converge in probability, use MGF and combine it with taylor series*]{.underline}

```{=tex}

Proof of Theorem 5.5.14: We will show that, for $|t|<h$, the mgf of $\sqrt{n}\left(\bar{X}_n-\mu\right) / \sigma$ converges to $e^{t^2 / 2}$, the mgf of a $\mathrm{n}(0,1)$ random variable.

Define $Y_i=\left(X_i-\mu\right) / \sigma$, and let $M_Y(t)$ denote the common mgf of the $Y_i \mathrm{~s}$, which exists for $|t|<\sigma h$ and is given by Theorem 2.3.15. Since
$$
\frac{\sqrt{n}\left(\bar{X}_n-\mu\right)}{\sigma}=\frac{1}{\sqrt{n}} \sum_{i=1}^n Y_i
$$
we have, from the properties of mgfs (see Theorems 2.3.15 and 4.6.7),
$$
\begin{aligned}
M_{\sqrt{n}\left(\hat{X}_n-\mu\right) / \sigma}(t) & =M_{\Sigma_{i=1}^n} Y_i / \sqrt{n}(t) \\
& =M_{\Sigma_{i=1}^n} Y_i\left(\frac{t}{\sqrt{n}}\right) \\
...
$$
where $M_Y^{(k)}(0)=\left.\left(d^k / d t^k\right) M_Y(t)\right|_{t=0}$. Since the mgfs exist for $|t|<h$, the power series expansion is valid if $t<\sqrt{n} \sigma h$.
...
$$
\lim _{n \rightarrow \infty} \frac{R_Y(t / \sqrt{n})}{(t / \sqrt{n})^2}=0
$$
Since $t$ is fixed, we also have
$$
\lim _{n \rightarrow \infty} \frac{R_Y(t / \sqrt{n})}{(1 / \sqrt{n})^2}=\lim _{n \rightarrow \infty} n R_Y\left(\frac{t}{\sqrt{n}}\right)=0,
$$
and (5.5.5) is also true at $t=0$ since $R_Y(0 / \sqrt{n})=0$. Thus, for any fixed $t$, we can write
$$
\begin{aligned}
\lim _{n \rightarrow \infty}\left(M_Y\left(\frac{t}{\sqrt{n}}\right)\right)^n & =\lim _{n \rightarrow \infty}\left[1+\frac{(t / \sqrt{n})^2}{2 !}+R_Y\left(\frac{t}{\sqrt{n}}\right)\right]^n \\
& =\lim _{n \rightarrow \infty}\left[1+\frac{1}{n}\left(\frac{t^2}{2}+n R_Y\left(\frac{t}{\sqrt{n}}\right)\right)\right]^n \\
& =e^{t^2 / 2}
\end{aligned}
$$
by an application of Lemma 2.3.14, where we set $a_n=\left(t^2 / 2\right)+n R_Y(t / \sqrt{n})$. (Note that (5.5.5) implies that $a_n \rightarrow t^2 / 2$ as $n \rightarrow \infty$.) Since $e^{t^2 / 2}$ is the mgf of the $n(0,1)$ distribution, the theorem is proved.
```
## Delta Methods
...
-   \href{https://vanderbilt365.sharepoint.com/:b:/s/contemporary.statistical.inference/EZ9WY0CEUaVPuEmFoxn4clwBnSIu6etovtXibsBcNLbvQA?e=qNXZ6I}{Bob's Homework 1}
...
$$
Ratio = (\frac{\theta}{1-\theta})^{x-y}(1-\theta)^{n-n'}*contant(x,y)
$$
...
This apply to all $\theta$ so we have $a=0$ and $b=0$, making $x=y$ and $n=n'$ $\blacksquare$
\end{itemize}
```
...
$$
\begin{gathered}
M_1 = \frac{1}{n}\sum X_i^1, \mu_1 = EX^1\\
M_2 = \frac{1}{n}\sum X_i^2, \mu_2 = EX^2\\
...
\end{gathered}
$$
...
$$
\bar X = \theta, \frac{1}{n}\sum X_i^2 = \sigma^2 + \theta^2
$$

$$
\Rightarrow \hat \theta = \bar X, \hat \sigma^2 = \frac{1}{n}\sum X_i^2 - \bar X^2 \blacksquare
$$
...
For random sample $\boldsymbol{Y} = Y_1,Y_2,...$, $Y_i\sim n(\mu_i,\phi)$,
such that

$$
l(\boldsymbol{\theta}|\boldsymbol{y}) = -n log(2\pi) - \frac{n}{2}log \phi - \frac{1}{2\phi} \sum(y_i-\mu_i)^2
$$
...
$$
l(\boldsymbol{\theta}|\boldsymbol{y}) = -n log(2\pi) - \frac{n}{2}log \phi - \frac{1}{2\phi} \sum(y_i-\beta x_i - \alpha)^2
$$
...
Solving we have $\hat\mu_i = \hat\alpha + \hat\beta x_i$ 
and $\hat \phi$
...
\href{https://vanderbilt365-my.sharepoint.com/personal/zhuohui_liang_vanderbilt_edu/_layouts/15/onedrive.aspx?id=%2Fsites%2Fcontemporary%2Estatistical%2Einference%2FShared%20Documents%2FExercise%20Sets%2FES3%2FES3%20Solutions%2Epdf&listurl=https%3A%2F%2Fvanderbilt365%2Esharepoint%2Ecom%2Fsites%2Fcontemporary%2Estatistical%2Einference%2FShared%20Documents&parentview=1}{Bob's Homework 3}
...
- In LRT chapter $\lambda = \frac{\sup f(x|\theta_0)}{\sup f(x|\theta)}$, where in NP lemma, $\lambda^\prime = \frac{\sup f(x|\theta_1)}{\sup f(x|\theta_0)}$, mark the difference.

- The rejection region of LRT is $R = \{x: \lambda(x) \le c\},c\in(0,1)$ vs rejection rejoin in the NP lemma: $R^\prime: \frac{\sup f(x|\theta_1)}{\sup f(x|\theta_0)} > k,k>0$, inverting the ratio we will have the same rejection region

- See \href{https://vanderbilt365.sharepoint.com/:b:/s/contemporary.statistical.inference/ESc9NuNBKdZCioHKKBV216ABmpGw9bYLCCK-VCA_wlZOJg?email=zhuohui.liang%40Vanderbilt.Edu&e=siQig5}{Bob's HW5}
...
- UMP is unbias(can't find a proof yet)
...
An LRT is a test that rejects $H_{0}$ for small values of $\lambda(\mathbf{x})$. From (8.2.2), the rejection region, $\{\mathbf{x}: \lambda(\mathbf{x}) \leq c\}$, can be written as

$$
\left\{\mathbf{x}:\left|\bar{x}-\theta_{0}\right| \geq \sqrt{-2(\log c) / n}\right\}
$$

As $c$ ranges between 0 and $1, \sqrt{-2(\log c) / n}$ ranges between 0 and $\infty$. Thus the LRTs are just those tests that reject $H_{0}: \theta=\theta_{0}$ if the sample mean differs from the hypothesized value $\theta_{0}$ by more than a specified amount.

The analysis in Example 8.2.2 is typical in that first the expression for $\lambda(\mathbf{X})$ from Definition 8.2.1 is found, as we did in (8.2.1). Then the description of the rejection region is simplified, if possible, to an expression involving a simpler statistic, $\left|\bar{X}-\theta_{0}\right|$ in the example.

\textbf{Example 8.2.3 (Exponential LRT)}

Let $X_{1}, \ldots, X_{n}$ be a random sample from an exponential population with pdf

$$
f(x \mid \theta)= \begin{cases}e^{-(x-\theta)} & x \geq \theta \\ 0 & x<\theta\end{cases}
$$

where $-\infty<\theta<\infty$. The likelihood function is

$$
L(\theta \mid \mathbf{x})=\left\{\begin{array}{ll}
e^{-\Sigma x_{i}+n \theta} & \theta \leq x_{(1)} \\
0 & \theta>x_{(1)} .
\end{array} \quad\left(x_{(1)}=\min x_{i}\right)\right.
$$

Consider testing $H_{0}: \theta \leq \theta_{0}$ versus $H_{1}: \theta>\theta_{0}$, where $\theta_{0}$ is a value specified by the experimenter. Clearly $L(\theta \mid \mathbf{x})$ is an increasing function of $\theta$ on $-\infty<\theta \leq x_{(1)}$. Thus, the denominator of $\lambda(\mathbf{x})$, the unrestricted maximum of $L(\theta \mid \mathbf{x})$, is

$$
L\left(x_{(1)} \mid \mathbf{x}\right)=e^{-\Sigma x_{i}+n x_{(1)}} .
$$

If $x_{(1)} \leq \theta_{0}$, the numerator of $\lambda(\mathbf{x})$ is also $L\left(x_{(1)} \mid \mathbf{x}\right)$. But since we are maximizing $L(\theta \mid \mathbf{x})$ over $\theta \leq \theta_{0}$, the numerator of $\lambda(\mathbf{x})$ is $L\left(\theta_{0} \mid \mathbf{x}\right)$ if $x_{(1)}>\theta_{0}$. Therefore, the likelihood ratio test statistic is

$$
\lambda(\mathbf{x})= \begin{cases}1 & x_{(1)} \leq \theta_{0} \\ e^{-n\left(x_{(1)}-\theta_{0}\right)} & x_{(1)}>\theta_{0}\end{cases}
$$

A graph of $\lambda(\mathbf{x})$ is shown in Figure 8.2.1. An LRT, a test that rejects $H_{0}$ if $\lambda(\mathbf{X}) \leq c$, is 8 test with rejection region $\left\{\mathbf{x}: x_{(1)} \geq \theta_{0}-\frac{\log c}{n}\right\}$. Note that the rejection region depends on the sample only through the sufficient statistic $X_{(1)}$. That this is generally the case will be seen in Theorem 8.2.4.

Example 8.2.3 again illustrates the point, expressed in Section 7.2.2, that differentiation of the likelihood function is not the only method of finding an MLE. In Example 8.2.3, $L(\theta \mid \mathbf{x})$ is not differentiable at $\theta=x_{(1)}$.

\textbf{Example 8.2.5 (LRT and sufficiency)}

In Example 8.2.2, we can recognize that $\bar{X}$ is a sufficient statistic for $\theta$. We could use the likelihood function associated with $\bar{X}\left(\bar{X} \sim \mathrm{n}\left(\theta, \frac{1}{n}\right)\right)$ to more easily reach the conclusion that a likelihood ratio test of $H_{0}: \theta=\theta_{0}$ versus $H_{1}: \theta \neq \theta_{0}$ rejects $H_{0}$ for large values of $\left|\bar{X}-\theta_{0}\right|$.

Similarly in Example 8.2.3, $X_{(1)}=\min X_{i}$ is a sufficient statistic for $\theta$. The likelihood function of $X_{(1)}$ (the pdf of $X_{(1)}$ ) is

$$
L^{*}\left(\theta \mid x_{(1)}\right)= \begin{cases}n e^{-n\left(x_{(1)}-\theta\right)} & \theta \leq x_{(1)} \\ 0 & \theta>x_{(1)} .\end{cases}
$$

This likelihood could also be used to derive the fact that a likelihood ratio test of $H_{0}: \theta \leq \theta_{0}$ versus $H_{1}: \theta>\theta_{0}$ rejects $H_{0}$ for large values of $X_{(1)}$.
```
...
\includepdf[page={42-44},pagecommand={\thispagestyle{fancy}}]{/Users/liang/Library/CloudStorage/GoogleDrive-jeffreyliang1995@gmail.com/My Drive/GoodNotes/Phd.2022 Spring/Note_for_Inference.pdf}

### Unbiased CI

\includepdf[page={45},pagecommand={\thispagestyle{fancy}}]{/Users/liang/Library/CloudStorage/GoogleDrive-jeffreyliang1995@gmail.com/My Drive/GoodNotes/Phd.2022 Spring/Note_for_Inference.pdf}

### Find CI by Pivoting CDF

-   More example in \href{https://vanderbilt365-my.sharepoint.com/:b:/g/personal/zhuohui_liang_vanderbilt_edu/Ebyd8nV9j4lEodyl8A5ReV0BICdwofUuyEzgBxnMXUETrw?e=V4YKut}{homework4}, that shit hard
-   Need more example
...
\includepdf[page={74-76},pagecommand={\thispagestyle{fancy}}]{/Users/liang/Library/CloudStorage/GoogleDrive-jeffreyliang1995@gmail.com/My Drive/GoodNotes/Phd.2022 Spring/Note_for_Inference.pdf}

## P-value

\includepdf[page={78-79},pagecommand={\thispagestyle{fancy}}]{/Users/liang/Library/CloudStorage/GoogleDrive-jeffreyliang1995@gmail.com/My Drive/GoodNotes/Phd.2022 Spring/Note_for_Inference.pdf}
...
|              |                             |                                     |                            |
|              | 2.  Finite Variance         | by CLT,                             |                            |
|              |                             |                                     |                            |
|              |                             | Lindeberg-Feller for identicality   |                            |
+--------------+-----------------------------+-------------------------------------+----------------------------+
| B            | 1.  Independent             | *Approx*                            | *Approx*                   |
|              |                             |                                     |                            |
|              | 2.  Finite Variance         | by CLT, Sluskey, and N large enough | By CLT                     |
|              |                             |                                     |                            |
|              | 3.  Identical               |                                     |                            |
+--------------+-----------------------------+-------------------------------------+----------------------------+
| C            | 1.  Independent             | *Approx*                            | *equal variance: Exact*    |
|              |                             |                                     |                            |
|              | 2.  Unknown Finite Variance | by LLN                              | *unequal variance: Approx* |
|              |                             |                                     |                            |
|              | 3.  Identical               |                                     |                            |
|              |                             |                                     |                            |
|              | 4.  $X_i$ Normality         |                                     |                            |
...
For inference $\beta$ - i.i.d - We don't need *Linear* if N sufficient large and X random - We don't need *Normality* since we have CLT - We don't need *Homoscadecity aka. equal variance* since we have robost variance estimator and N sufficient large

For sub-group specific mean $E[Y|X]$ - i.i.d - Linear - We don't need *normality* - We don't need *Homoscadecity aka. equal variance* since we have robust variance estimator and N sufficient large

For prediction $E[\hat Y|X]$ - i.i.d - Linear - normality - Homoscadecity
...
=E[(X^TX)^{-1}X^Ty|X]\\
= (X^TX)^{-1}X^TE[y|X] = (X^TX)^{-1}X^TX\beta\\
=\beta
\end{gathered}
$$
...
Assumption: 1. error terms are pairwise independent 2. $Var[Y|X = x] = \sigma^2$ for all x (homoscedacity), this assumption can be elimated with robust estimator

$$
\begin{aligned}
Var(\hat \beta) 
&= Var[E[\hat \beta|X]] + E[Var(\hat \beta|X)] &
\mbox{~ total variance}\\
&= \cancel{Var(\beta)} + E[Var(\hat \beta|X)] &
\mbox{~ constant}\\
&= E[Var((X^TX)^{-1}X^Ty|X)] &
\mbox{~ Linearity}\\
&= E[(X^TX)^{-1}X^T*Var(y|X)*((X^TX)^{-1}X^T)^T] &
 \mbox{~ algreba}\\
&= E[(X^TX)^{-1}X^T*(\sigma^2I)*((X^TX)^{-1}X^T)^T] &
 \mbox{~ homoscedacity}\\
&= E[\sigma^2(X^TX)^{-1}]
\end{aligned}
$$
...
**For** $E[Y|X] = \beta_0 + \beta_1x$

$c\beta_1$ denotes difference in mean Y between subgroups differing in X by c units.

**For** $E[log(Y)|X] = \beta_0 + \beta_1 (x)$

$\beta_1 log(1+q)\Rightarrow$ Difference in mean Y between subgroups differing in X by (100 × q)%

**For** $E[log(Y)|X] = \beta_0 + \beta_1x$
...
-   $1.6^{\beta_1} \Rightarrow$ Geometric mean ratio of Y compareing subgroup differing in X by $exp(log(1.6)\beta_1) \rightarrow log(1.6)$ unit

## Multivariate Regression

### Confounding/Percision Variable
...
E[Y|X=x,Z=z] = \alpha_0 + \alpha_1X + \alpha_2Z\\
\Rightarrow\hat \beta_1 = \hat \alpha_1\\
\Rightarrow Var(\hat \beta_1) \ge Var(\hat \alpha)
\end{gathered}
$$
...
-   (3+1) cubic functions, each has 4 parameters
-   for the head and tail cubic function, restricted to linear function so each free up 2 degrees of freedom $\sum^3_{i=0} \beta_ix^i \Rightarrow \beta_0 + \beta_1x$
-   3 constrains at each knots, ie. continuous, differentiable, and have continuous first derivative at each knot
...
**Gauss-Markov Theorem**: Correct specified weighted least square estimator has the minumum variance ammong all **unbiased linear estimator** of $\beta$, if normality, linearity, no collinearity and finite variance assumptions are satisfied.

aka. Best Linear Unbiased Estimator.

-   The OLS estimator is the best (i.e., minimum variance) linear (in y) unbiased (for $\beta$) estimator of $\beta$ under the assumptions of the Gauss-Markov theoerm if the errors are homoscedastic.
...
-   A model is saturated if the number of parameters and constrains add up to the number of groups
...
$$
logit(P(Y\ge1|X=x)) = \alpha_0 + \alpha_1 I(x\ge 1)
$$

-   This is a logistic model, and we have 4 groups ($\{x =0, x\ge 1\}\cap\{Y=0,Y\ge1\}$), and we have 2 paramteres and 2 constrains ($P(Y\ge1|X=x) + P(Y=0|X=x) = 1$) so the model is saturated.

$$
log(\frac{P(Y=k|x=x)}{P(Y=0|X=x)})
= \beta_{0k} + \beta_{1k}I(x=1)+\beta_{2k}I(x=2), k=1,2
$$

-   This is a multinomial model, we have 9 groups(3\*3), and the model have 6 parameter($\beta_{ij}$) and 3 constrains $\sum_k P(Y=k|X=x) = 1$, so the model is saturated.

$$
logit(P(Y\le k|X=x)) = \gamma_{0k} - \gamma_1 I(x=1) - \gamma_2 I(x=2), k = 0,1
$$
...
Note: $\underbrace{p(1-p)^{x-1}}_{\mbox{n trials}}$ or $\underbrace{p(1-p)^{x}}_{n+1\mbox{ trials}}$
